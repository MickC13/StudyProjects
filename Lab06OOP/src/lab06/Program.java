 ...
 * Extended with XML operations for saving and loading client data.
...
public class Program {
     ...
     * Includes XML operations for data persistence.
     */
    public void show() {
        mainFrame = new JFrame("Pochta Rossii - XML Operations");
        ...
        // Create toolbar with buttons including XML operations
        ...
        JButton saveXMLButton = createButton("SAVE.png", "Save to XML");
        JButton loadXMLButton = createButton("LOAD.png", "Load from XML");
        
        setupButtonListeners(addButton, editButton, deleteButton, searchButton, saveXMLButton, loadXMLButton);
    ...
    /**
     * Sets up event listeners for all interface buttons including XML operations.
     */
    private void setupButtonListeners(JButton addButton, JButton editButton, 
                                     JButton deleteButton, JButton searchButton,
                                     JButton saveXMLButton, JButton loadXMLButton) {
    ...
    /**
     * Saves table data to XML file using DOM parser.
     * Creates structured XML document with client information.
     */
    private void saveDataToXML() {
        FileDialog saveDialog = new FileDialog(mainFrame, "Save Client Data as XML", FileDialog.SAVE);
        saveDialog.setFile("clients.xml");
        ...
        try {
            ...
            // Create root element
            Element rootElement = doc.createElement("postal_clients");
            doc.appendChild(rootElement);
            
            // Add clients from table
            for (int i = 0; i < model.getRowCount(); i++) {
                ...
                name.appendChild(doc.createTextNode(model.getValueAt(i, 0).toString()));
                client.appendChild(name);
                
                // Add phone
                Element phone = doc.createElement("phone");
                phone.appendChild(doc.createTextNode(model.getValueAt(i, 1).toString()));
                client.appendChild(phone);
                
                // Add address
                Element address = doc.createElement("address");
                address.appendChild(doc.createTextNode(model.getValueAt(i, 2).toString()));
                client.appendChild(address);
                
                // Add newspaper
                Element newspaper = doc.createElement("newspaper");
                newspaper.appendChild(doc.createTextNode(model.getValueAt(i, 3).toString()));
            ...
            transformer.transform(new DOMSource(doc), new StreamResult(new File(fullPath)));
            
            JOptionPane.showMessageDialog(mainFrame, 
                "Data successfully saved to XML: " + fileName, 
                "XML Save Complete", 
                JOptionPane.INFORMATION_MESSAGE);
                
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(mainFrame, 
                "Error saving XML file: " + ex.getMessage(), 
                "XML Save Error", 
                JOptionPane.ERROR_MESSAGE);
            ex.printStackTrace();
        }
    ...
    /**
     * Loads table data from XML file using DOM parser.
     * Parses XML structure and populates the table with client data.
     */
    private void loadDataFromXML() {
        FileDialog loadDialog = new FileDialog(mainFrame, "Load Client Data from XML", FileDialog.LOAD);
        ...
        try {
            // Create document builder
            DocumentBuilder builder = DocumentBuilderFactory.newInstance().newDocumentBuilder();
            Document doc = builder.parse(new File(fullPath));
            ...
            for (int i = 0; i < clientList.getLength(); i++) {
                ...
                if (clientNode.getNodeType() == Node.ELEMENT_NODE) {
                    ...
                    // If elements are empty, try attributes
                    if (name.isEmpty()) name = clientElement.getAttribute("name");
                    if (phone.isEmpty()) phone = clientElement.getAttribute("phone");
                    if (address.isEmpty()) address = clientElement.getAttribute("address");
                    if (newspaper.isEmpty()) newspaper = clientElement.getAttribute("newspaper");
                    
                    // Add to table if we have basic data
            ...
            JOptionPane.showMessageDialog(mainFrame, 
                "Data successfully loaded from XML: " + fileName + "\nLoaded " + loadedCount + " clients", 
                "XML Load Complete", 
                JOptionPane.INFORMATION_MESSAGE);
            ...
            JOptionPane.showMessageDialog(mainFrame, 
                "Error loading XML file: " + ex.getMessage(), 
                "XML Load Error", 
                JOptionPane.ERROR_MESSAGE);
            ex.printStackTrace();
        }
